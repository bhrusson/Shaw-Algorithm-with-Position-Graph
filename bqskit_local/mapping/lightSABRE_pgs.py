...
DEFAULT_LIGHTSABRE_HEURISTIC = 'lookahead+decay'
...
class GeneralizedLightSABREAlgorithmPGS(GeneralizedSabreAlgorithmPGS):
    ...
    def routed_trial_selection_score(self, circuit: Circuit) -> int:
        """Return the Qiskit-like trial selection score."""
        return self.routed_swap_count(circuit)
    ...
    def _score_swap(
        ...
        """Score a candidate swap using direct frontier/extended-set evaluation."""
        pos1, pos2 = swap
        use_critical_path = self._uses_critical_path()
        old_front_scores = None
        if use_critical_path:
            old_front_scores = {
                point: self._get_distance_from_mapping(
                    tuple(int(q) for q in circuit[point].location),
                    pgs.logical_to_position,
                    D,
                )
                for point in F
            }
        ...
        try:
            ...
            # Qiskit's LightSABRE wrapper builds the basic term with
            # SetScaling.Constant, i.e. it uses the front-layer sum rather than
            # averaging by the frontier size.
            front = front_total
