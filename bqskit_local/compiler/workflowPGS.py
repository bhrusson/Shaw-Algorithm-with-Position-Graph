...
class WorkflowPGS(BasePassPGS, Sequence[BasePassPGS]):
    ...
    def __init__(self, passes: WorkflowLike, name: str = '') -> None:
        ...
        """
        if isinstance(passes, WorkflowPGS):
            self._passes: list[BasePassPGS] = copy.deepcopy(passes._passes)
            self._name: str = name if name else copy.deepcopy(passes._name)
            return

        if isinstance(passes, BasePassPGS):
            passes = [passes]
        ...
        if not all(isinstance(p, BasePassPGS) for p in passes):
            truth_list = [isinstance(p, BasePassPGS) for p in passes]
    ...
    @staticmethod
    def load(filename: str) -> WorkflowPGS:
        import pickle
    ...
    def is_workflow(workflow: WorkflowLike) -> bool:
        if not is_iterable(workflow):
            return isinstance(workflow, BasePassPGS)
        return all(isinstance(p, BasePassPGS) for p in workflow)

    def __str__(self) -> str:
        name_seq = f'Workflow: {self.name}\n\t'
        pass_strs = [
            f'{i}. Workflow: {p.name if isinstance(p, WorkflowPGS) else p}'
            for i, p in enumerate(self._passes)
        ]
        return name_seq + '\n\t'.join(pass_strs)

    def __add__(self, other: WorkflowLike) -> WorkflowPGS:
        return WorkflowPGS(self._passes + WorkflowPGS(other)._passes)

    def __radd__(self, other: WorkflowLike) -> WorkflowPGS:
        return WorkflowPGS(WorkflowPGS(other)._passes + self._passes)

    def __len__(self) -> int:
        return self._passes.__len__()

    def __iter__(self) -> Iterator[BasePassPGS]:
        return self._passes.__iter__()

    @overload
    def __getitem__(self, _key: int, /) -> BasePassPGS:
        ...

    @overload
    def __getitem__(self, _key: slice, /) -> list[BasePassPGS]:
        ...

    def __getitem__(self, _key: int | slice) -> BasePassPGS | list[BasePassPGS]:
...
WorkflowLike = Union[WorkflowPGS, Iterable[BasePassPGS], BasePassPGS]