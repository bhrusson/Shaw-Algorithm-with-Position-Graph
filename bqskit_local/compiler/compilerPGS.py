...
from bqskit_local.compiler.taskPGS import CompilationTaskPGS
from bqskit_local.compiler.workflowPGS import WorkflowPGS
from bqskit_local.compiler.workflowPGS import WorkflowLike
...
class CompilerPGS:
    ...
    def __enter__(self) -> CompilerPGS:
    ...
    def submit(
        ...
        task = CompilationTaskPGS(circuit, WorkflowPGS(workflow))
    ...
    def result(self, task_id: uuid.UUID) -> Circuit | tuple[Circuit, PassDataPGS]:
    ...
    def compile(
        ...
        max_logging_depth: int = ...,
        data: MutableMapping[str, Any] | None = ...,
    ) -> Circuit | tuple[Circuit, PassDataPGS]:
        ...
...
def sigint_handler(signum: int, frame: FrameType, compiler: CompilerPGS) -> None:
...
_compiler_instances: set[CompilerPGS] = set()
