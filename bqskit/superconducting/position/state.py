...
class PositionGraphState:
    def __init__(
        ...
        self._logical_to_position = np.full(self.num_qudits, -1, dtype=int)
        self._position_to_logical = np.full(len(pg.position_labels), -1, dtype=int)
    ...
    def __copy__(self) -> "PositionGraphState":
        return self.copy()

    def __deepcopy__(self, memo: dict[int, object]) -> "PositionGraphState":
        ...
        clone = PositionGraphState(
            self._pg,
            radices=copy.deepcopy(tuple(self.radices), memo),
            gateSet=self._gateSet,
        )
        memo[id(self)] = clone
        clone._logical_to_position[:] = self._logical_to_position
        clone._position_to_logical[:] = self._position_to_logical
    ...
    def assert_consistent(self) -> None:
        for q, p in enumerate(self.logical_to_position):
            ...
            if self.position_to_logical[p] != q:
                raise RuntimeError(
                    f"Inconsistent mapping: logical {q} â†’ physical {p}, "
                    f"but physical {p} â†’ logical {self.position_to_logical[p]}"
                )
    ...
    def __str__(self):
        return (
            f"logicalâ†’pos {self._logical_to_position}\n"
            f"posâ†’logical {self._position_to_logical}"
        )
