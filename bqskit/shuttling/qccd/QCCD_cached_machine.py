...
class QCCDMachineModel(MachineModel):
    ...
    def __init__(self,
        ...
        self._shortest_path_tree_cache: dict[int, dict[int, list[int]]] = {}
        self._longest_move_path_length_cache: int | None = None
        self.timing_mat = []
        self.calculate_timing_matrix()

    def calculate_timing_matrix(self) -> None:
        self._all_pair_travelling_time_cache = None
        self._shortest_path_tree_cache = {}
        self._longest_move_path_length_cache = None
        self.timing_mat = [
    ...
    def get_shortest_path_tree(self, source: int) -> Sequence[Sequence[int]]:
        """Return a cached shortest-path tree rooted at source."""
        source = int(source)
        cached = self._shortest_path_tree_cache.get(source)
        if cached is None:
            cached = self.position_graph.get_shortest_path_tree(source)
            self._shortest_path_tree_cache[source] = cached
        return cached
    ...
    def get_move_path(self, source: int, target: int) -> list[int]:
        ...
        if source == target:
            return [source]
        return list(self.get_shortest_path_tree(source)[target])

    def get_move_neighbors(self, position: int) -> list[int]:
        """Return adjacent move-reachable positions."""
        return list(self.position_graph.get_neighbors_of(int(position)))

    def get_longest_move_path_length(self) -> int:
        """Return the longest finite shortest-path length in the position graph."""
        if self._longest_move_path_length_cache is None:
            longest_path = 0
            for source in range(self.position_graph.num_qudits):
                for path in self.get_shortest_path_tree(source):
                    longest_path = max(longest_path, len(path))
            self._longest_move_path_length_cache = int(longest_path)
        return self._longest_move_path_length_cache
