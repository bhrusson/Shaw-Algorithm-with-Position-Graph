...
TEST_GATE = Operation(CCXGate(), (0, 13, 16))
...
def benchmark(machine_cls: type, mapping_cls: type, name: str) -> dict[str, object]:
    ...
    times: list[float] = []
    final_moves: list[tuple[int, int]] = []
    ...
    for _ in range(5):
        ...
        final_moves = [(int(a), int(b)) for a, b in moves]
        final_changed = {
...
def summarize(result: dict[str, object]) -> None:
    ...
    print(f"  leading_moves_first20: {result['leading_moves']}")
...
def main() -> None:
    ...
    print(f"  same_num_moves_simple: {cg_result['num_moves'] == simple_result['num_moves']}")
    print(f"  same_num_moves_native: {cg_result['num_moves'] == pgs_result['num_moves']}")
    print(f"  same_leading_moves_simple: {cg_result['leading_moves'] == simple_result['leading_moves']}")
    print(f"  same_leading_moves_native: {cg_result['leading_moves'] == pgs_result['leading_moves']}")
    print(
